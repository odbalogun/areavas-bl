...
class CategoryListByProduct(Resource):
    @staticmethod
    @swag_from('../docs/category-list-by-product/GET.yml')
    def get(prod):
        schema = CategorySchema(many=True)
        product = ProductRepository.get(prod)
        if not product:
            return response_error(ERROR_RESOURCE_DOES_NOT_EXIST, "Product does not exist", status=404)
        return response_success(paginate(CategoryRepository.get_all_by_product(product.id), schema))